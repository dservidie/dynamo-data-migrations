...
describe('migrationsDb', () => {
    ...
    describe('configureMigrationsLogDbSchema()', () => {
        it('should resolve when no errors are thrown while creating migrationsLogDb', async () => {
            AWSMock.mock(
                'DynamoDB',
                'createTable',
                (_params: CreateTableInput, callback: (error: Error | null) => void) => {
                    callback(null);
                },
            );
            AWSMock.mock(
                ...
                (
                    _state: 'tableExists',
                    _params: DescribeTableInput,
                    callback: (error: Error | null, responseObj: { pk: string; sk: string }) => void,
                ) => {
                    callback(null, { pk: 'foo', sk: 'bar' });
                },
            );
            await expect(
                migrationsDb.configureMigrationsLogDbSchema(new AWS.DynamoDB({ apiVersion: '2012-08-10' }), 'DEV'),
        ...
        it('should reject when error is thrown while creating migrationsLogDb', async () => {
            ...
            await expect(
                migrationsDb.configureMigrationsLogDbSchema(new AWS.DynamoDB({ apiVersion: '2012-08-10' }), 'DEV'),
            ).rejects.toThrowError('Could not create table Migrations_Log');
        });
        ...
        it('should reject when error is thrown while waiting for migrationsLogDb table to be active', async () => {
            ...
            AWSMock.mock(
                ...
                (
                    _state: 'tableExists',
                    _params: DescribeTableInput,
                    callback: (error: Error | null, responseObj: { pk: string; sk: string } | null) => void,
                ) => {
            ...
            await expect(
                migrationsDb.configureMigrationsLogDbSchema(new AWS.DynamoDB({ apiVersion: '2012-08-10' }), 'DEV'),
            ).rejects.toThrowError('Table is not active');
        });
    });
    ...
    describe('deleteMigrationFromMigrationsLogDb()', () => {
        it('should resolve when no errors are thrown while deleting migration', async () => {
            ...
            await expect(
                migrationsDb.deleteMigrationFromMigrationsLogDb(
                    item,
                    new AWS.DynamoDB({ apiVersion: '2012-08-10' }),
                    'DEV',
                ),
        ...
        it('should reject when error is thrown while deleting migration', async () => {
            ...
            ).rejects.toThrow('Could not delete migration');
        });
    });
    ...
    describe('doesMigrationsLogDbExists()', () => {
        it('should resolve when no errors are thrown while describing migrationsLogDb', async () => {
            AWSMock.mock(
                'DynamoDB',
                'describeTable',
                (_params: DescribeTableInput, callback: (error: Error | null) => void) => {
                    callback(null);
                },
            );
            await expect(
                migrationsDb.doesMigrationsLogDbExists(new AWS.DynamoDB({ apiVersion: '2012-08-10' }), 'DEV'),
            ).resolves.toBeTruthy();
        });
    ...
    describe('getAllMigrations()', () => {
        it('should return a migrations array', async () => {
            ...
            AWSMock.mock(
                'DynamoDB',
                'scan',
                (_params: ScanInput, callback: (error: Error | null, responseObj: { Items: ItemList }) => void) => {
                    callback(null, { Items });
                },
            );
        ...
        it('should make recursive calls and return the data of all recursive calls in single array', async () => {
            ...
            AWSMock.mock(
                'DynamoDB',
                'scan',
                (_params: ScanInput, callback: (error: Error | null, stub: sinon.SinonStub) => void) => {
                    callback(null, stub());
                },
            );
            const migrations = await migrationsDb.getAllMigrations(
